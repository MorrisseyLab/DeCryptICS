...
# Input a svs file. Output goes to file.

# Find clone option. Currently only works for positive cytoplasmic marks.
# TODO, loss of mark.
def SegmentImage(img_or_file_name, deconv_mat_use, clones = True, folder_out = None, multi_core = True):        
    # Check if svs_file
    if type(img_or_file_name) is str:
        obj_svs  = getROI_svs(img_or_file_name)
        img_or_file_name = obj_svs.getROI_img();
    #plot_img(img_or_file_name, hold_plot=True)
    out_seg = SegmentMPAS(img_or_file_name, None)   
    return out_seg
