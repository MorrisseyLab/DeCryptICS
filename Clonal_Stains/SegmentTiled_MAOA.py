...
from Clonal_Stains.SegmentCryptAndCloneMAOA   import SegmentMAOA
...
from deconv_mat import deconv_mat_MAOA
...
def GetThresholdsPrepareRun_MAOA(folder_in, file_in, folder_out):
    ## Make file name
    ...
    xyhw = auto_choose_ROI(file_name, deconv_mat_MAOA, plot_images = False)
    img_for_thresh = getROI_img_vips(file_name, xyhw[0], xyhw[1])
    thresh_cut_nucl, thresh_cut_nucl_blur, th_clone = calculate_thresholds(img_for_thresh, deconv_mat_MAOA)
    ...
    with open(full_folder_out + '/params.pickle', 'wb') as f:
        pickle.dump([folder_in, file_in, ROI_zoom, all_indx, deconv_mat_MAOA, thresh_three], f)      

def SegmentFromFolder_MAOA(folder_name):
    ...
    for i in range(x_tiles):
        for j in range(y_tiles):
            ...
            (img_segm_ii, clones_img_ii, crypt_features_ii, clone_features_ii, summary_ii, 
                 cluster_clones_ii, crypt_cnt_ii, mPAS_cluster_cnt_ii) = SegmentMAOA(img, thresh_cut)
