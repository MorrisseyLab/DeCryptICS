...
def GetThresholdsPrepareRun_MPAS(folder_in, file_in, folder_out):
    ## Make file name
    ...
    xyhw = auto_choose_ROI(file_name, deconv_mat_MPAS, plot_images = False)
    img_for_thresh = getROI_img_vips(file_name, xyhw[0], xyhw[1])
    thresh_cut_nucl, thresh_cut_nucl_blur, th_clone = calculate_thresholds(img_for_thresh, deconv_mat_MPAS)
    ...
    with open(full_folder_out + '/params.pickle', 'wb') as f:
        pickle.dump([folder_in, file_in, ROI_zoom, all_indx, deconv_mat_MPAS, thresh_three], f)      

def SegmentFromFolder_MPAS(folder_name):
    pkl_file    = open(folder_name + '/params.pickle', 'rb')
    folder_in, file_in, ROI_zoom, all_indx, deconv_mat_MPAS, thresh_cut = pickle.load(pkl_file)
    pkl_file.close()
    ...
    for i in range(x_tiles):
        for j in range(y_tiles):
            ...
            (img_segm_ii, clones_img_ii, crypt_features_ii, clone_features_ii, summary_ii, 
                 cluster_clones_ii, crypt_cnt_ii, mPAS_cluster_cnt_ii) = SegmentMPAS(img, thresh_cut)
...
class ImageFromFolder:
    def __init__(self, folder_name):
        ...
        pkl_file.close()
        self.file_name  = folder_in + "/" + file_in + ".svs"
        self.x_tiles    = len(all_indx)
        self.y_tiles    = len(all_indx[0])
        self.deconv_mat = deconv_mat
        self.thresh_cut = thresh_cut
        self.all_indx   = all_indx
        
    def openImage(self, i_j):
        i = i_j[0]
        j = i_j[1]
        xy_vals     = (int(self.all_indx[i][j][0]), int(self.all_indx[i][j][1]))
        wh_vals     = (int(self.all_indx[i][j][2]), int(self.all_indx[i][j][3]))
        img         = getROI_img_vips(self.file_name, xy_vals, wh_vals)
        return img
