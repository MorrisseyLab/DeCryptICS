...
import cv2
import numpy as np
from MiscFunctions            import col_deconvol, col_deconvol_and_blur, plot_img
...
from deconv_mat               import deconv_mat_KDM6A
...
from Clonal_Stains.func_negspace_crypt_clone import *
from Segment_clone_from_crypt import find_clones_negspace
## needed due to clone_thresh being argument in crypt contour finding
...
def SegmentKDM6A(img, thresh_cut):
    ...
    if thresh_cut is None:
        thresh_cut_nucl, thresh_cut_nucl_blur, th_clone = calculate_thresholds(img, deconv_mat_KDM6A)
    else:
        thresh_cut_nucl, thresh_cut_nucl_blur, th_clone = thresh_cut #thresh_stringent, thresh_lax 

    ## Colour deconvolution
    smallBlur_img_nuc, blurred_img_nuc, blurred_img_clone = col_deconvol_and_blur(img, deconv_mat_KDM6A, (11, 11), (37, 37), (27, 27))
    ...
    ## Process KDM6A clones
    ###########################################
    ## These aren't needed here but are arguments in functions below:
    ...
    # Find clone contours by comparing nuclear/clonal halo and halo gap
    clone_cnt = find_clones_negspace(crypt_cnt, nuclei_ch_raw, clone_ch_raw, backgrd, smallBlur_img_nuc)

    ## This should be done on the full slide contour list, especially in the case of 
    ## very small 1024x1024 DNN tiles as patch could stretch beyond single tile:
