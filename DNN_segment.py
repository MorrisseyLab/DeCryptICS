...
#   fn_l = ["DFC_ST_070220_A.svs", "DFC__DT_07022020_B.svs", "DFC_ST_07022020_C.svs", "DFC_ST_07022020_D.svs"]
   fn_l = ["/home/doran/Work/images/agne/DFC_ST_070220_A.svs",
           "/home/doran/Work/images/agne/DFC__DT_07022020_B.svs",
           "/home/doran/Work/images/agne/DFC_ST_07022020_C.svs",
           "/home/doran/Work/images/agne/DFC_ST_07022020_D.svs"]
   fta_l = ["/home/doran/Work/images/agne/Analysed_slides/Analysed_DFC_ST_070220_A/",
            "/home/doran/Work/images/agne/Analysed_slides/Analysed_DFC__DT_07022020_B/",
            "/home/doran/Work/images/agne/Analysed_slides/Analysed_DFC_ST_07022020_C/", 
            "/home/doran/Work/images/agne/Analysed_slides/Analysed_DFC_ST_07022020_D/"]
   cK = 0
   for file_name, folder_to_analyse in zip(fn_l,fta_l):
      ## Tiling
      if file_name.split('.')[-1]=='svs':
         ...
         if (choice.shape[0]==0):
            ...
            dwnsmpl_lvl = np.where(abs(mpp_test-mpp_fin/2.) < errlevel )[0][0]
            numpyrdown = 1 # could be generalised away from just a single down sampling
            size = (params.input_size_run*(4*dwnsmpl_lvl)*(2*numpyrdown), params.input_size_run*(4*dwnsmpl_lvl)*(2*numpyrdown))
            scaling_val = slide.level_dimensions[0][0] / float(slide.level_dimensions[dwnsmpl_lvl][0] / (2.*numpyrdown))
   #         all_indx = get_tile_indices(slide.level_dimensions[dwnsmpl_lvl], overlap = 50*(2*numpyrdown), SIZE = size)
            all_indx = get_tile_indices(slide.level_dimensions[0], overlap = 50*(4*dwnsmpl_lvl)*(2*numpyrdown), SIZE = size)
         # if img errors, load up full thing and tile/downsample from there
         bigimg = getROI_img_osl(file_name, (0,0), slide.level_dimensions[0], level = 0)
         numpyrdown = numpyrdown + 2*dwnsmpl_lvl
   #   else:
   ...
   #   all_indx = get_tile_indices(img_full.shape, overlap = 50, SIZE = size)
      ...
      for i in range(x_tiles):
         for j in range(y_tiles):
            xy_vals = int(all_indx[i][j][0]), int(all_indx[i][j][1])
            wh_vals = int(all_indx[i][j][2]), int(all_indx[i][j][3])
            img = bigimg[xy_vals[1]:(xy_vals[1]+wh_vals[1]), xy_vals[0]:(xy_vals[0]+wh_vals[0])]
            if (STREAM==1):
               xy_x = xy_vals[0]
               xy_y = xy_vals[1]
               for k in range(int(numpyrdown)):
   ...
   #            x_batch = keras.utils.to_channels_first(x_batch)
            ...
            # Perform prediction and find contours
            predicted_mask_batch = model.predict(x_batch)
            newcnts = mask_to_contours_withdilation(predicted_mask_batch, prob_thresh, clone_prob_thresh, chan_num, dwnsmpl_lvl) # add lower prob_thresh for clonal channels to increase sensitivity?
            for k in range(predicted_mask_batch.shape[chan_num]):
               newcnts[k] = [cc for cc in newcnts[k] if len(cc)>4] # throw away points and lines (needed in contour class)
...
def mask_to_contours_withdilation(preds, prob_thresh, clone_thresh, chan_num, dwnsamp):
   n_class = preds.shape[chan_num]
   all_class_cnts = []
   for j in range(n_class): # ensure correct number of elements
      all_class_cnts.append([])
   for j in range(n_class):
      ...
      for i in range(preds.shape[0]):
         ...
         _, mask = cv2.threshold(pred, thresh*255, 255, cv2.THRESH_BINARY)
         if dwnsamp==0:
            mask = cv2.morphologyEx(mask, cv2.MORPH_DILATE, st_7, iterations = 1)
            mask = cv2.morphologyEx(mask, cv2.MORPH_DILATE, st_5, iterations = 1)
         if dwnsamp==1:
            mask = cv2.morphologyEx(mask, cv2.MORPH_DILATE, st_3, iterations = 1)
            mask = cv2.morphologyEx(mask, cv2.MORPH_DILATE, st_3, iterations = 1)            
         # find contours
         cnts, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2:]
         contours += cnts
