...
#model.add(layers.Input(shape=(32,32,256)))
#model.add(SubpixelConv2D(scale=2))
#model.output_shape
...
def strde_sepconv_unet_xcept_gn_shallow(input_shape=(1024, 1024, 3), chan_num=3):
   ...
   x = sep_conv_layer_gn(x, 32, strides=1, shape=3, groups=4)
   x = sep_conv_layer_gn(x, 32, strides=1, shape=3, groups=4)
   x = dwnsmp_add_residual(x, down1, 32)
   # 256
   down2 = x
   
   x = sep_conv_layer_gn(x, 64, strides=1, shape=3, groups=4)
   x = sep_conv_layer_gn(x, 64, strides=1, shape=3, groups=4)
   x = dwnsmp_add_residual(x, down2, 64)
   # 128
   down3 = x
   
   x = sep_conv_layer_gn(x, 128, strides=1, shape=3, groups=8)
   x = sep_conv_layer_gn(x, 128, strides=1, shape=3, groups=8)
   x = dwnsmp_add_residual(x, down3, 128)
   # 64
   down4 = x
   
   x = sep_conv_layer_gn(x, 256, strides=1, shape=3, groups=16)
   x = sep_conv_layer_gn(x, 256, strides=1, shape=3, groups=16)
...
#   feature_maps = upsample_layer_gn(x, 32, strides=1, shape=3, groups=4, name = 'fmaps')
   feature_maps = sep_conv_layer_gn(x, 32, strides=1, shape=3, groups=4, name = 'fmaps')
...
#   x = upsample_layer_gn(x, 16, strides=1, shape=3, groups=2)
   x = sep_conv_layer_gn(x, 16, strides=1, shape=3, groups=2)
...
def strde_sepconv_unet_xcept_gn_deep(input_shape=(1024, 1024, 3), chan_num=3):
   inputs = layers.Input(shape=input_shape)
   # 1024
   x = conv_layer_gn(inputs, 16, strides=2, shape=3, groups=2)
   # 512
   ...
   x = sep_conv_layer_gn(x, 384, strides=1, shape=3, groups=24)
   x = sep_conv_layer_gn(x, 384, strides=1, shape=3, groups=24)
   x = dwnsmp_add_residual(x, down5, 384)
   # 16
   down6 = x
   
   x = sep_conv_layer_gn(x, 512, strides=1, shape=3, groups=32)
   x = sep_conv_layer_gn(x, 512, strides=1, shape=3, groups=32)
