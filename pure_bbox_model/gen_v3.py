...
def bbox_xcycwh(cnti):
    bb_cv2 = cv2.boundingRect(cnti)
    # x,y,w,h
    return np.array([bb_cv2[0]+bb_cv2[2]/2, bb_cv2[1]+bb_cv2[3]/2, bb_cv2[2], bb_cv2[3]])
    
def box_overlap_x1y1x2xy2(A,B):
    return (A[0] < B[2] and A[2] > B[0] and A[1] < B[3] and A[3] > B[1])

def box_overlap_xcycwh(A,B):
    return ((A[0]-A[2]/2) < (B[0]+B[2]/2) and (A[0]+A[2]/2) > (B[0]-B[2]/2) and (A[1]-A[3]/2) < (B[1]+B[3]/2) and (A[1]+A[3]/2) > (B[1]-B[3]/2))
      
def get_bbox_prob_from_masks(masks, max_crypts = 400, max_gen_crypts = 100): # increase this?
   mask_cp     = masks[:, :, 0]
   ...
   bboxes = np.array([bbox_xcycwh(cnti) for cnti in contours])

   dummy_mask = np.ones(crypt_mask.shape, dtype=np.uint8)*255
   for i in range(bboxes.shape[0]):
      dummy_mask = cv2.rectangle(dummy_mask, (int(bboxes[i,0]-bboxes[i,2]/2), int(bboxes[i,1]-bboxes[i,3]/2)), (int(bboxes[i,0]+bboxes[i,2]/2), int(bboxes[i,1]+bboxes[i,3]/2)), -255, -1)
   ...
   gen_bboxes = np.array([bbox_xcycwh(cnti) for cnti in gen_cnts])
   genb_areas = np.array([box[2]*box[3] for box in gen_bboxes])
   # cut large boxes and reduce number for intersect comparison
   gen_bboxes = gen_bboxes[np.where(genb_areas<2500)[0],:]
   ...
   for i in range(len(gen_bboxes)):
      intersects.append(np.all([not box_overlap_xcycwh(gen_bboxes[i], box_j) for box_j in bboxes]))
   gen_bboxes = gen_bboxes[np.where(np.array(intersects))[0]]

   # Find which channel is use, pull that channel
   stuff_mask = masks[:,:,0:] # now include a crypt class at position 0
   # make prob
   p_i = []
   for dim_i in range(stuff_mask.shape[2]):
     ...
     p_i.append(p_feat_j)
   p_no_object = np.zeros(p_i[-1].shape, dtype=np.float32)
   p_i.append(p_no_object)
   ...
   null_bbox = np.array([[0.99, 0.99, 0.015, 0.015]])
   null_p_i = np.zeros((1, p_i.shape[1]), dtype=np.float32)
   null_p_i[0,-1] = 1 # set null box as no object class
   ...
   # stack a row indicating the number of true and generated crypts on bboxes
   # and an empty labels row on p_i
   header_bbox = np.array([[n_crypts + n_gen_crypts, 0, 0, 0]])
   header_p_i = np.zeros((1, p_i.shape[1]))
   return np.vstack([header_bbox, bboxes]), np.vstack([header_p_i, p_i])

def batch_process_mask(batch_masks, max_crypts = 400):
    b_bboxes  = []
    b_p_i     = []
    for batch_i in range(batch_masks.shape[0]):     
        bboxes, p_i = get_bbox_prob_from_masks(batch_masks[batch_i], max_crypts = max_crypts)
        b_bboxes.append(bboxes)
        b_p_i.append(p_i)            
    return np.array(b_bboxes), np.array(b_p_i)
...
class DataGen_curt(Sequence):
   ...
   def __getitem__(self, idx):
      ...
      bboxes, p_i = batch_process_mask(y_batch, max_crypts=self.num_bbox)
      # in order: crypt, clone, partial, fufi, no object
      return x_batch.astype(np.float32), bboxes.astype(np.float32), p_i.astype(np.float32)
